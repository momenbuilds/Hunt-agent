...
2. `./.hunt-agent/skills/` (project-local — scoped to the repo)
3. `~/.hunt-agent/builtin-skills/` (installer-managed shipped skills)
4. `~/.hunt-agent/skills/` (personal)
5. any dirs passed via `--skills <dir>` or the `skills_dirs` config

Use `~/.hunt-agent/skills/` for personal skills you want available
everywhere, or the project-local `./.hunt-agent/skills/` for skills
...
- **Scaffold:** run `/skills new <name>` in the TUI — it writes a templated
  `./.hunt-agent/skills/<name>/SKILL.md` and loads it immediately.
