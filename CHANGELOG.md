...
First public release of Hunt-agent — an open-source AI pentest agent for
authorized security testing by security engineers, professional penetration
testers, and bug hunters.
...
- **Gemini provider support** — first-class `gemini` backend, native
  ...
  interactive API-key setup, recommended model ordering, and `cheap cost` tags
  for low-cost Gemini models in the picker.
- **Burp bridge runtime** — `hunt-agent --burp [port]`, `/burp [port]`, Burp
...
- **MCP integration** via `@modelcontextprotocol/sdk`, including one-flag Browser
  MCP and a standalone `hunt-agent-browser-mcp` stdio server with a local
...
- **Configuration** — zod-validated `~/.hunt-agent/config.json` with
