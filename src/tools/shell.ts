...
import { checkURLInScope, extractURLs } from '../scope/policy.js';
...
export class ShellTool implements Tool {
  ...
  async run(args: Record<string, unknown>, signal: AbortSignal, _p: Prompter): Promise<string> {
    ...
    for (const url of extractURLs(cmdStr)) {
      const scope = checkURLInScope(url);
      if (!scope.ok) throw new Error(`scope policy blocked shell command: ${scope.reason}`);
    }
