...
import { checkURLInScope } from '../scope/policy.js';
...
export class HTTPTool implements Tool {
  ...
  async run(args: Record<string, unknown>, signal: AbortSignal, _p: Prompter): Promise<string> {
    ...
    const scope = checkURLInScope(resolved);
    if (!scope.ok) throw new Error(`scope policy blocked http request: ${scope.reason}`);
    ...
    if (!headers.has('user-agent')) {
      headers.set('user-agent', 'hunt-agent/0.1');
    }
