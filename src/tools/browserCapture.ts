// browser_capture_* tools. Surface traffic the Hunt-agent Chrome
...
export class BrowserCaptureStatusTool extends BaseCaptureTool {
  ...
  description(): string {
    return 'Show counts and last-activity time for traffic captured by the Hunt-agent Chrome extension. Call this first to confirm the extension is connected and forwarding before relying on the other browser_capture_* tools.';
  }
...
export class BrowserCaptureBurpTasksTool extends BaseCaptureTool {
  ...
  description(): string {
    return 'List scan / plan / scope tasks queued from the Hunt-agent Burp extension. Use this after the user sends requests from Burp to decide what to scan or plan next.';
  }
  schema(): Record<string, unknown> {
    return { type: 'object', properties: {} };
  }
...
export class BrowserCaptureBurpIssuesTool extends BaseCaptureTool {
  ...
  description(): string {
    return 'List Hunt-agent issues exposed to the Burp extension for import into Burp Scanner issues.';
  }
  schema(): Record<string, unknown> {
    return { type: 'object', properties: {} };
  }
  async run(): Promise<string> {
    ...
    if (issues.length === 0) return 'No Hunt-agent issues queued for Burp import.';
    return JSON.stringify(
