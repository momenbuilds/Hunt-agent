// Config file at ~/.hunt-agent/config.json. Loaded once at startup; saved atomically (write to
...
const ProviderKind = z.enum(['api', 'openai-compatible', 'local-llm', 'local-cli']);
const ProviderConfig = z.object({
  ...
  command: z
    .string()
    .default('')
    .refine((s) => s === '' || noShellMeta(s), {
      message: 'providers[].command must not contain shell metacharacters',
    }),
  args: z.array(z.string()).default([]),
  inputMode: z.enum(['stdin', 'argument']).default('stdin'),
...
const ModelRoutes = z.object({
  ...
  'skill-writer': z.string().optional(),
  memory: z.string().optional(),
  cheap: z.string().optional(),
  'large-context': z.string().optional(),
});
export type ModelRoutes = z.infer<typeof ModelRoutes>;

const FallbackRoutes = z.object({
  default: z.array(z.string()).optional(),
  planner: z.array(z.string()).optional(),
  executor: z.array(z.string()).optional(),
  verifier: z.array(z.string()).optional(),
  reporter: z.array(z.string()).optional(),
  summarizer: z.array(z.string()).optional(),
  'skill-writer': z.array(z.string()).optional(),
  memory: z.array(z.string()).optional(),
  cheap: z.array(z.string()).optional(),
  'large-context': z.array(z.string()).optional(),
});
export type FallbackRoutes = z.infer<typeof FallbackRoutes>;

const LocalCLIConfig = z.object({
  timeoutMs: z.number().int().positive().default(120_000),
  maxOutputBytes: z.number().int().positive().default(200_000),
  allowInteractive: z.boolean().default(false),
});
export type LocalCLIConfig = z.infer<typeof LocalCLIConfig>;
...
const ConfigSchema = z.object({
  display_name: z.string().default('Hunt'),
  ...
  fallbacks: FallbackRoutes.default({}),
  local_cli: LocalCLIConfig.default({}),
...
export function configPath(): string {
  const override = process.env.HUNT_AGENT_CONFIG;
  if (override && override.length > 0) return override;
  const home = homedir();
  return join(home, '.hunt-agent', 'config.json');
}

/** Warn (once) when the old ~/.pentesterflow/config.json exists but the new
 *  ~/.hunt-agent/config.json does not. Does NOT auto-migrate — the user
 *  should run the migration guide at docs/migration.md. */
export function checkLegacyConfig(): string | null {
  const home = homedir();
  if (!home) return null;
  const oldPath = join(home, '.pentesterflow', 'config.json');
  const newPath = join(home, '.hunt-agent', 'config.json');
  if (existsSync(oldPath) && !existsSync(newPath)) {
    return `[hunt-agent] Legacy config detected at ${oldPath}.\n  Run: cp -r ~/.pentesterflow ~/.hunt-agent  to migrate.\n  See docs/migration.md for the full migration guide.`;
  }
  return null;
}
...
export async function save(cfg: Config): Promise<void> {
  ...
  const tmp = join(dir, '.hunt-agent.cfg.tmp');
