...
export interface AgentRunOptions {
  ...
  tools?: boolean;
  /** Model route role requested for this turn. */
...
export class Agent {
  ...
  async saveContextSnapshot(reason = 'periodic'): Promise<string> {
    ...
    const out: string[] = [];
    out.push('# Hunt-agent Session Context');
    out.push('');
   ...
   *
   * Persisting to ~/.hunt-agent/config.json is the caller's job —
  ...
  private isToolAllowed(toolName: string): { ok: boolean; reason?: string } {
    ...
    // hunt-agent registers tools under two names (Unix and
  ...
  private async runInner(
    ...
    const role = opts?.role ?? (opts?.tools === false ? 'planner' : 'executor');
