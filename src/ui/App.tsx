...
/** Persist a disabled-skills list change (writes ~/.hunt-agent/config.json). */
export type PersistDisabledSkills = (names: string[]) => Promise<void>;
export type ConfigCommand = (args: string[]) => Promise<string>;
...
export interface AppProps {
  ...
  /** Run provider/model config subcommands that need registry/config access. */
  runProviderCommand?: ConfigCommand;
  runModelCommand?: ConfigCommand;
  ...
  /** Persist /skills enable/disable to ~/.hunt-agent/config.json. */
  ...
  /** Start the local Burp/hunt-agent bridge on demand from /burp. */
...
function handleSlash(
  ...
  readConfig: () => { backend: Backend; baseURL: string; apiKey: string; model: string },
  applyProvider: ApplyProvider,
  runProviderCommand: ConfigCommand | undefined,
  runModelCommand: ConfigCommand | undefined,
  ...
  switch (cmd) {
      ...
      void agent
        ...
        .then((coverageContext) =>
          runAgentTurn(buildCoverageNextPrompt(objective, coverageContext), {
            ...
            runOptions: { tools: false, role: 'planner' },
          }),
        )
        .catch((err: unknown) =>
          dispatch({
    ...
    case '/provider':
      if (rest.length > 0) {
        runConfigSubcommand('/provider', rest, runProviderCommand, dispatch);
        return true;
      }
    ...
    case '/model': {
      ...
      if (['use', 'route', 'routes', 'fallback', 'status', 'test', 'bench'].includes(subcommand)) {
        runConfigSubcommand('/model', rest, runModelCommand, dispatch);
        return true;
      }
    ...
    case '/verify': {
      const prompt = buildVerifyPrompt(rest);
      runAgentTurn(prompt, {
        transcriptUserText: raw.trim(),
        systemText: 'finding verification — tools disabled',
        runOptions: { tools: false, role: 'verifier' },
      });
      return true;
    }
    case '/target': {
...
function runConfigSubcommand(
  label: string,
  args: string[],
  runner: ConfigCommand | undefined,
  dispatch: React.Dispatch<Action>,
): void {
  if (!runner) {
    dispatch({
      type: 'append',
      entry: { kind: 'error', text: `${label}: config commands are unavailable in this runtime` },
    });
    return;
  }
  void runner(args)
    .then((text) =>
      dispatch({
        type: 'append',
        entry: { kind: 'system', text },
      }),
    )
    .catch((err: unknown) =>
      dispatch({
        type: 'append',
        entry: { kind: 'error', text: `${label}: ${(err as Error).message}` },
      }),
    );
}
...
function buildVerifyPrompt(args: string[]): string {
  ...
  const findingId = args
    .filter((_arg, idx) => idx !== modelsIdx && (modelsIdx < 0 || idx <= modelsIdx))
    .join(' ')
    .trim();
  return [
    findingId
      ? `Verify finding candidate: ${findingId}`
      : 'Verify the strongest current candidate finding from the conversation.',
    modelList.length ? `Requested verifier models: ${modelList.join(', ')}` : '',
    '',
    'You are in finding-verification mode for this turn.',
    '',
    'Rules:',
    '- Do not call tools, run commands, fetch URLs, scan targets, or modify files in this turn.',
    '- Reason only over captured evidence, prior tool outputs, confirmed scope, and safe conversation context.',
    '- Try to disprove the finding first: missing reproduction, ambiguous response, out-of-scope host, weak impact, false positive, scanner-only evidence, or unsupported severity.',
    '- If the evidence survives, say verification_status: agrees and list the exact evidence references.',
    '- If the evidence fails, say verification_status: rejected and explain what is missing.',
    '- If evidence is insufficient but plausible, say verification_status: needs-human-review and list the smallest safe checks that require user approval before active testing.',
  ]
    .filter(Boolean)
    .join('\n');
}
 ...
 * Persistence to ~/.hunt-agent/config.json is delegated to the
...
function handleSkillsCommand(
  ...
  // ./.hunt-agent/skills/<name>/SKILL.md (an auto-discovered dir), then
  ...
  if (rest[0] === 'new') {
    ...
    const skillsRoot = resolve(process.cwd(), '.hunt-agent', 'skills');
    ...
    try {
      ...
      // even if .hunt-agent/skills didn't exist at startup.
...
const KEYBINDINGS: Array<{ keys: string; desc: string }> = [
  ...
  { keys: 'Ctrl-C', desc: 'quit hunt-agent' },
];
...
function buildHelpText(
  ...
  out.push(c.bold.cyan('Hunt-agent') + c.gray(' — quick reference'));
...
function openProviderPicker(
  ...
  const req: AskRequest = {
    question: {
      header: 'provider',
      question: 'Which LLM backend should hunt-agent use?',
      options: [
        ...
        dispatch({
          ...
          entry: {
            ...
            text:
              ...
              'or pre-set them in ~/.hunt-agent/config.json, then run /provider again.',
          },
        });
        return;
      }
