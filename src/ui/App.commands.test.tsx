...
describe('UI slash commands (terminal integration)', () => {
  ...
  it('/plan runs a plan-only turn with tools disabled', async () => {
    ...
    expect(runSpy.mock.calls[0][3]).toEqual({ tools: false, role: 'planner' });
  ...
  it('/verify runs a verifier-routed tools-disabled turn', async () => {
    mounted = renderApp();
    await tick();
    await submit(mounted.stdin, '/verify finding-123 --models claude-code,codex-cli');
    await tick();

    expect(runSpy).toHaveBeenCalledTimes(1);
    expect(runSpy.mock.calls[0][0]).toContain('finding-verification mode');
    expect(runSpy.mock.calls[0][0]).toContain('finding-123');
    expect(runSpy.mock.calls[0][0]).toContain('claude-code, codex-cli');
    expect(runSpy.mock.calls[0][3]).toEqual({ tools: false, role: 'verifier' });
  });

  it('/provider list and /model routes use config command callbacks', async () => {
    const runProviderCommand = vi.fn(async () => 'providers ok');
    const runModelCommand = vi.fn(async () => 'routes ok');
    mounted = renderApp({ runProviderCommand, runModelCommand });
    await tick();

    await submit(mounted.stdin, '/provider list');
    expect(runProviderCommand).toHaveBeenCalledWith(['list']);
    expect(mounted.lastFrame()).toContain('providers ok');

    await submit(mounted.stdin, '/model routes');
    expect(runModelCommand).toHaveBeenCalledWith(['routes']);
    expect(mounted.lastFrame()).toContain('routes ok');
    expect(runSpy).not.toHaveBeenCalled();
  });
});
